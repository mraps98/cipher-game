...
document.addEventListener("DOMContentLoaded",function(){
    ...
    function vigenereEncode(word,keyword) {
      ...
      function vigenereDecode(word,keyword) {
        var alphabet = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
        var decryptWord = "";
        for (var i = 0;i < word.length;i++) {
    ...
    // Function for generating next Question
     function generateNextQuestion(){
        ...
        const outputString = vigenereEncode(inputString,key);
        rightAnswerIndex = Math.floor(Math.random()*4);
        console.log(rightAnswerIndex);
        questionText.innerHTML = `Encode '${inputString}' using Vigenere with Key ${key}`;
